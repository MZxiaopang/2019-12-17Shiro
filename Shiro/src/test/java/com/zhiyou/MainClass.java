...
public class MainClass {
	...
	@Test
	public void shiro() {
		String username="zhangsan";//假装用户输入的账号
		String password="123";//假装用户输入的密码
		// 1：加载realm配置文件  用来初始化securityManagerFactory
		Factory<org.apache.shiro.mgt.SecurityManager> factory= new IniSecurityManagerFactory("classpath:realm.ini");
		...
		Subject subject = SecurityUtils.getSubject();
		 ...
		 System.out.println("用户具有role1角色："+hasRole);
		 
		 // 判断用户是否具有多个角色
		 boolean hasAllRoles = subject.hasAllRoles(Arrays.asList("role1","role2"));
		 
		 System.out.println("用户具有role1角色，role2角色"+hasAllRoles);
		 ...
		 System.out.println("用户具有user:select,user:add,user:delete权限:"+permittedAll);
		 //退出
		 subject.logout();
		 // 判断是否认证成功
